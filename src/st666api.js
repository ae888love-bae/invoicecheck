"use strict";
/**
 * AE Internal API Client — Deposit Remark via deposits/search
 *
 * Endpoint thật:
 * GET /deposits/search
 *
...
const BASE    = process.env.AE_API_BASE || "https://boapi.da77ae888.com/ae888-ims/api/v1";
const BO_USER = process.env.AE_BO_USER;
const BO_PASS = process.env.AE_BO_PASS;
...
function buildHeaders(token) {
  return {
    ...
    "Referer": process.env.AE_REFERER || "https://bo.da77ae888.com",
    "X-Currency": "VND2",
...
async function searchDepositsByStatus(username, statusType, dayRange = 1) {
  ...
  logger.info("AE deposits/search", {
    username,
    statusType,
    dayRange,
    results: list.length,
  ...
  if (!list.length) {
    logger.info("AE empty response sample", {
...
async function fetchPendingRemark(username) {
  ...
  try {
    ...
    if (credited.length > 0) {
      ...
      if (minutesAgo < 30) {
        logger.info("AE deposit already credited", {
          username,
    ...
    if (!list.length) {
      logger.warn("AE no deposits found", { username });
      return null;
    }
    ...
    logger.info("AE deposit selected", {
      username,
    ...
    logger.error("AE fetchPendingRemark error", {
      username,
