...
/**
 * BO Browser — AE888 version
 * Login qua Playwright lấy cookies,
...
const BO_ORIGIN    = process.env.BO_ORIGIN     || "https://bo.da77ae888.com";
...
async function getSession() {
  ...
  try {
    ...
    // ── AE888 Login selectors ─────────────────────────────────────────────────
    // Từ DevTools: username nằm trong div.login-input-box.input-username
    // password có data-testid="login-password"
    const userSel = await Promise.race([
      page.waitForSelector('.login-input-box.input-username input',  { state: "visible", timeout: 30_000 }),
      page.waitForSelector('input[data-testid="login-userid"]',      { state: "visible", timeout: 30_000 }),
      page.waitForSelector('input[placeholder="User Name"]',         { state: "visible", timeout: 30_000 }),
      page.waitForSelector('input[type="text"].formik-input',        { state: "visible", timeout: 30_000 }),
    ]).catch(() => null);
    ...
    // Password selector — AE888 dùng data-testid="login-password"
    await Promise.race([
      page.fill('input[data-testid="login-password"]', BO_PASSWORD),
      page.fill('input[type="password"].formik-input',  BO_PASSWORD),
      page.fill('input[placeholder="Password"]',        BO_PASSWORD),
    ]).catch(async () => {
      // Fallback: lấy input password đầu tiên visible
      const pwSel = await page.waitForSelector('input[type="password"]', { state: "visible", timeout: 5_000 });
      if (pwSel) await pwSel.fill(BO_PASSWORD);
    });

    // Click Login button — AE888 dùng div.form-btn > button hoặc button:has-text("Login")
    const loginBtn = await Promise.race([
      page.waitForSelector('.form-btn button',          { state: "visible", timeout: 5_000 }),
      page.waitForSelector('button:has-text("Login")',  { state: "visible", timeout: 5_000 }),
      page.waitForSelector('button[type="submit"]',     { state: "visible", timeout: 5_000 }),
    ]).catch(() => null);
...
async function searchByStatus(session, username, statusType, dayRange = 1) {
  ...
  const res = await axios.get(`${BO_API_BASE}/deposits/search`, {
    params: {
      ...
      exactmatch: true,
      statusType,
      zoneType:   process.env.BO_ZONE || "ASIA_HO_CHI_MINH",
